...
1. You will need to use [Linux/UNIX command line](https://documentation.ubuntu.com/desktop/en/latest/tutorial/the-linux-command-line-for-beginners/) in order to navigate the environment. Once logged into SciNet, navigate to the following folder using the cd command to change the current directory: `cd /project/restricted/mdl/revelio`
